...
func TestGenerateArithmeticMutationsV2(t *testing.T) {
	...
	ast.Inspect(file, func(n ast.Node) bool {
		mutations = append(mutations, GenerateArithmeticMutationsV2(n, fset, content, source, &mutationID)...)
		return true
	})
...
func TestIsArithmeticOpV2(t *testing.T) {
	tests := []struct {
		op       token.Token
		expected bool
	}{
	...
	for _, tt := range tests {
		t.Run(tt.op.String(), func(t *testing.T) {
			if result := isArithmeticOpV2(tt.op); result != tt.expected {
				t.Fatalf("isArithmeticOpV2(%v) = %v, expected %v", tt.op, result, tt.expected)
			}
		})
	}
}
