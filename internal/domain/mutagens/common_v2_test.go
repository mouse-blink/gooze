...
func TestOffsetForPosV2(t *testing.T) {
	...
	offset, ok := offsetForPosV2(fset, literalPos)
	if !ok {
...
func TestReplaceRangeV2(t *testing.T) {
	original := []byte("abcde")
	mutated := replaceRangeV2(original, 1, 4, "XYZ")

	if string(mutated) != "aXYZe" {
		t.Fatalf("replaceRangeV2 result = %q, expected %q", string(mutated), "aXYZe")
	}

	unchanged := replaceRangeV2(original, 10, 12, "nope")
