...
func TestViewCmd_PositionalArgOverridesOutputFlag(t *testing.T) {
	mockWorkflow := domainmocks.NewMockWorkflow(t)
	...
	mockWorkflow.On("View", mock.MatchedBy(func(args domain.ViewArgs) bool {
		return args.Reports == m.Path("./custom-reports")
	})).Return(nil)

	cmd.SetArgs([]string{"--output", ".gooze-reports", "view", "./custom-reports"})
	err := cmd.Execute()
	require.NoError(t, err)
}
